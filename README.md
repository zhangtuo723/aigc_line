...
- **多种节点**：支持图片、图片编辑、视频、音频、视频放大和 3D 导演台节点；生成片段直接由 video 节点承载，片段内生成 Shot 写入提示词；导演台另存可编辑的白模预演 Shot。
- **Excalidraw 图片编辑台**：把一个或多个已有输出的图片节点连入图片编辑节点，全部素材会作为可选、可移动的普通图片进入全屏 Excalidraw；可叠加画笔、图形、箭头和文字，框选或 Shift 多选后右键导出，外部画布会自动新增相连的只读图片输出节点。
...
图片编辑节点用于在不修改源图的前提下组合和标注画布素材。创建节点后，将一个或多个已有 `sourcePath` 的图片节点连接到它，再点击“打开图片编辑器”。连接图片会同时出现在 Excalidraw 工作区中，并保持普通图片元素的选择、移动、缩放和旋转能力。

编辑器支持自由画笔、橡皮擦、矩形、圆形、箭头、连线和文字等 Excalidraw 工具。需要输出时，先框选或按 Shift 多选图片与标注元素，再右键选择“导出所选素材”。应用会把所选元素合成为 PNG，写入 `generated/image-edits/`，随后在外部 React Flow 画布创建一个只读图片节点，并自动连接为“图片编辑节点 → 导出图片节点”。同一编辑会话可重复导出多个不同组合；右上角不提供整张工作区保存按钮。

## 3D 导演台

3D 导演台用于在正式图片和视频生成前完成低成本的空间验证：先用白模确认场景体量、人物站位、行走路线、镜头位置和画幅，再把构图截图作为画布参考继续生成。导演工程直接保存在 `director` 节点中，不依赖外部 DCC 软件。
