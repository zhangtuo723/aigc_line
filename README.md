...
- **3D 导演台**：在画布节点中打开虚拟片场，添加演员、群众和几何道具，控制站位、姿势、显隐、锁定、机位、FOV、Roll 与画幅；导演视角可用移动/旋转控制器直接拖拽当前帧机位，支持多 Shot 站位快照，以及由镜头时长驱动的 24fps 时间线播放、拖动预览和相机关键帧插值。构图截图和 WebM 预演视频均保留地面网格，自动保存到项目并生成相连、不可重新生成的只读图片或视频节点。
