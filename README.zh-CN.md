...
AIGC CANVAS 是一个面向完整 AI 短剧生产闭环的 Harness Engineering 桌面工作台。用户输入剧本后，Agent 会为每个角色先生成唯一四联身份底图，再通过图生图派生不同场景/服装版本，同时生成无人俯视全景场景图，组织多模态参考并调用 ComfyUI 生成视频，再读取实际视频逐段分析、验证和推动修正。
...
- **Qwen 音视频验证**：提交 video 节点 ID（或只关联一个下游视频的 image 节点 ID），系统自动解析视频、提示词和参考素材，并由 Qwen3.5-Omni Plus 联合读取画面与音轨，自由组织审核报告；Agent 根据实际证据、问题和建议自行决定接受、修复或重做。
...
- **多种节点**：支持图片、视频、音频和视频放大节点；生成片段直接由 video 节点承载，剧情规划、旁白、时间线和片段内 Shot 写入图片或视频提示词；音频节点可上传并预览本地音频。
- **上传与项目资产**：左侧工具栏可上传本地图片、视频和音频并自动创建对应节点；资产面板集中查看当前项目生成及上传的素材，支持按类型筛选并拖拽回画布复用。
- **节点化生产流水线**：Agent 直接创建“参考图片 → 视频”节点链，画布节点是生产数据的唯一来源。
- **多模型图片生成**：ComfyUI 提供 Krea 2 Turbo、Z-Image Turbo；云端支持 Google Nano Banana 2 / Pro 与火山方舟 Doubao-Seedream-5.0-pro / lite，覆盖 16:9 / 1:1 / 4:3 的 2K 文生图和单参考图图生图。
...
- **通用视频分析**：Agent 可把项目内视频路径或公开 HTTP(S) 视频地址连同自定义分析要求交给 Qwen3.5-Omni Plus，联合分析完整画面与音轨并返回带时间戳的 Markdown 报告。
- **RTX 视频放大**：视频放大节点支持 2× / 3× / 4× 和 FAST / MEDIUM / HIGH / ULTRA 质量档位，输出帧率自动跟随源视频。
- **媒体预览**：图片直接展示，生成的视频可以在画布节点内播放；工作区协议支持视频 Range 流式读取。
- **系统配置**：宽屏分组展示基础服务与 AI 云服务；可配置 ComfyUI、Agent、Google AI、火山方舟 Seedream、Qwen3.5-Omni Plus 和默认生图模型。Google/Qwen 密钥使用 Electron 系统安全存储，Seedream Key 明文保存在本机设置。
- **项目持久化**：聊天记录、画布布局、节点参数和生成产物均按项目保存。

## 创作流程

1. 新建项目并选择本地工作目录。
2. 输入剧本、梗概或对白并调用内置短剧 Skill；Agent 默认忠于核心剧情进行艺术化视听深化，补足戏剧节拍、动作反应、潜台词、必要对白/旁白和镜间衔接；只有明确要求时才逐字逐场还原。
3. Agent 调用独立人物/场景 Skill：先生成并审核每个角色唯一的头部+正侧背四联底图，再由底图图生图派生不同场景/服装版本；同时生成无人俯视全景场景图。
4. Agent 先按戏剧节拍拆分可独立生成和审核的 5/10/15 秒视频片段，再为每个 video 节点设计一个或多个带时间范围的内部 Shot；画布不再创建独立分镜节点。
...
6. Agent 调用 Qwen3.5-Omni 联合读取实际视频和音轨，依据审查报告逐片段验证角色、服装、环境、动作、内部 Shot、切点、台词/旁白、口型和音效；不合格节点按审核结果修正并重做。
...
| 工作流 | 类型 | 输出 |
|---|---|---|
| Krea 2 Turbo | 文生图 | 直接生成 2K |
| Z-Image Turbo | 文生图 | 直接生成 2K |
| Nano Banana 2（Google API） | 文生图 / 图生图，2K | — |
| Nano Banana Pro（Google API） | 文生图 / 图生图，2K | — |
| Doubao-Seedream-5.0-pro（火山方舟 API） | 文生图 / 图生图，2K | — |
| Doubao-Seedream-5.0-lite（火山方舟 API） | 文生图 / 图生图，2K | — |
...
- 火山方舟 ARK_API_KEY（Doubao-Seedream-5.0-pro / lite 共用）及可配置 API Base URL
- 默认生图模型或工作流
