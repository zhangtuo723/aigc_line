...
| `electron/main/services/agent/codex-subagents.ts` / `codex-subagent-reader.ts` | Codex 子 Agent 协作事件适配与受限只读子会话读取；统一折叠卡片、执行过程、结果及任务状态 |
| `electron/main/services/agent/subagent-tasks.ts` | Claude 子任务生命周期：处理 SDK task_started/progress/updated/notification，持久化并推送子 Agent 运行状态 |
...
| `electron/main/services/project-index-db.ts` | 应用级 `projects.sqlite` 唯一 `projects` 表：历史 `projects.json` 首次事务迁移、项目查询/分页、最近打开状态；历史同目录重复记录保留，原 JSON 保留不再写入 |
| `scripts/migrate-projects.mjs` | 手动、幂等的历史项目索引迁移脚本，`pnpm migrate:projects --data-dir "<Electron userData>/aigc-line"` |
...
| `src/shared/chat-subagents.ts` / `src/components/ChatSubagentGroup.tsx` | 按父 Agent 工具调用归集子 Agent 回复与工具步骤，聊天中以默认折叠、展开后限高滚动的面板展示 |
...
- **Codex 子 Agent 展示**：识别原生 exec JSONL 的 `collab_tool_call`（SDK 类型声明尚未覆盖），按 `receiver_thread_ids` 和 `agents_states` 将每个子 Agent 的任务状态、调度步骤与结果放入共享的默认折叠、最高 440px 内滚动面板。`spawn_agent` 工具完成和 `wait` 超时不代表子任务完成；关闭已完成子 Agent 保留完成结果，`send_input`/`followup_task` 后防止上一轮完成快照覆盖新任务状态。详情通过同版本只读 app-server 的 `thread/read` 与分页 `thread/turns/list` 读取，只接受当前 SDK 报告的子会话 ID，校验父会话与项目目录并过滤继承的父会话历史；不调用 `thread/resume`/`turn/start`，不触发额外模型回合。1 秒串行刷新、失败退避，已完成详情停止刷新，退出/中断后关闭读取进程。只读进程把未完成且未加载的 turn 标为 `interrupted`，不得据此推断实际子任务已停止；运行状态以 exec 事件为准，读取到完成/失败结果才补充终态，SDK 进程结束后仍未完成的任务收束为已停止。详情读取失败保留状态/结果摘要并明确提示；消息与归属继续写入项目聊天 JSONL，重开项目复核 Claude/Codex 活动子任务，旧聊天不补录。

- **子 Agent 状态**：新 Claude 子任务的折叠面板头部常驻显示“等待中 / 运行中 / 已暂停 / 已完成 / 失败 / 已停止”。任务状态通过 SDK 生命周期事件记录到聊天 JSONL，独立于 Agent 工具调用状态；工具返回或主 Agent 回合结束不得把后台子任务标为完成，也不得中断仍运行的子任务工具展示。内部单个工具失败后恢复不影响子任务最终完成状态。聊天分页仍从已加载记录读取对应任务状态；没有生命周期记录时显示“启动中”或“状态未知”，不推断历史任务已完成。会话流结束或应用重启后没有活动会话的未完成任务显示“已停止”，重新打开仍有活动会话的项目保留真实运行状态。普通 Bash/工作流后台任务不生成子 Agent 面板，生命周期记录不计入对话条数；旧聊天不补录状态。
...
- **项目管理**：新建项目弹窗选择名称、Claude Code / Codex、模型（动态列表/默认/自定义）及目录；应用级 `projects.sqlite` 的唯一 `projects` 表与项目 `.aigc-line/manifest.json` 保存 `agent.provider/model`，缺失字段的历史项目默认 Claude Code。模型列表加载期间显示转圈动画与加载提示，并暂时禁用模型下拉框；完成或失败后恢复选择。模型列表使用 `chat:listModels` IPC；Claude 从 SDK `supportedModels()` 获取，Codex SDK 无模型发现方法，列表查询使用只读 app-server `model/list`，子 Agent 详情另使用只读 `thread/read` 与 `thread/turns/list`。同目录重复创建或已有清单时拒绝覆盖；创建、删除、设置最近打开项目及索引查询通过同一事务队列串行；旧 `projects.json` 仅首次事务导入 SQLite，保留原文件且后续不再写入；目录/清单写入成功后才更新索引。创建/打开/删除项目，每项目一个本地目录，聊天历史、画布快照、生成产物都持久化在项目内。应用启动时恢复退出前仍打开的项目；用户点击项目页“返回”时，先刷新待保存编辑，再通过 `project:close` IPC 按项目 ID 在索引事务中清除 `lastOpenedId`，成功后清空当前工作区并回首页，下次启动不再自动打开；保存或索引写入失败时保留项目并提示重试，旧项目关闭请求不得清除新项目的恢复目标。普通列表刷新或删除项目时保持当前页面，不触发自动导航。首页使用最大 1920px 的响应式项目网格，突出项目名称，分层展示目录、Agent、模型和创建日期；`project:search` 从 SQLite 按名称/目录/Agent/模型模糊查询，每页 18 个项目，默认按最新创建排序，也可按中文名称排序；打开与删除按钮独立，操作失败在首页显示错误。
